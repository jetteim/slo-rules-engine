...
module SloRulesEngine
  module Datadog
    class StatePlanner
      ...
      def plan_operations(manifest, state:)
      ...
      def diff_operations(manifest, state:)
        resolved_slo_ids = resolved_slo_ids_from_state(state)
        ARTIFACTS.flat_map do |spec|
          collection(manifest, spec.fetch(:collection)).each_with_index.map do |artifact, index|
      ...
      def orphan_backend_resource_findings(manifest, managed_state)
        prune_operations(manifest, managed_state).each_with_index.map do |operation, _index|
          {
      ...
      def diff_operation_for(manifest, artifact, index, spec, state, resolved_slo_ids)
        ...
        action = if backend_state.nil?
                   'create'
                 elsif recreate_monitor?(artifact, spec, name, state, resolved_slo_ids)
                   'recreate'
                 elsif changes.empty?
                   'noop'
                 else
      ...
      def recreate_monitor?(artifact, spec, name, state, resolved_slo_ids)
